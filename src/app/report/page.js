      ...
      <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        {step === 1 && (
          <>
            ...
            <div>
              <label style={{ display: 'block', fontWeight: 600, fontSize: '14px', marginBottom: '8px' }}>What type of issue?</label>
            ...
            <div>
              <label style={{ display: 'block', fontWeight: 600, fontSize: '14px', marginBottom: '8px' }}>How severe?</label>
              <div style={{ display: 'flex', gap: '8px' }}>
                {SEVERITY_OPTIONS.map(opt => (
                  <button key={opt.value} onClick={() => update('severity', opt.value)} style={{
                    flex: 1, padding: '10px 6px', borderRadius: '8px',
                    border: `1.5px solid ${form.severity === opt.value ? '#2D6A4F' : '#E5E7EB'}`,
                    background: form.severity === opt.value ? '#F0FDF4' : 'white', cursor: 'pointer', textAlign: 'center'
                  }}>
                    <div style={{ fontWeight: 700, fontSize: '13px', color: form.severity === opt.value ? '#2D6A4F' : '#374151' }}>{opt.label}</div>
                    <div style={{ fontSize: '11px', color: '#6B7280', marginTop: '2px', lineHeight: 1.3 }}>{opt.desc}</div>
                  </button>
                ))}
              </div>
            </div>
